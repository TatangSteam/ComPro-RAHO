...
export default function ArticleListingClient({ initialArticles }: { initialArticles: Article[] }) {
  return (
    <Suspense fallback={<ArtikelKesehatanFallback />}>
      <ArtikelKesehatanContent initialArticles={initialArticles} />
    </Suspense>
  );
}
...
function ArtikelKesehatanContent({ initialArticles }: { initialArticles: Article[] }) {
  ...
  useEffect(() => {
    if (initialArticles.length === 0) fetchArticles();
